...
#     gpdm = fullfilename[-12:-4] #股票代码获取带交易所
    ...
    cursor = db.cursor()            
    sql = '''
CREATE TABLE if not exists DAYZS(
    DM varchar(10),
...
'''
#     sql = '''if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[D1%s]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
# DROP TABLE [dbo].[D1%s]
# ''' % (dm,dm)
    cursor.execute(sql)
    cursor.close()
...
#     try:
#         cursor = db.cursor()            
#         sql = "SELECT max(JYRQ) as JYRQ FROM DAYZS;" % (dm)
#         cursor.execute(sql)
#         row = cursor.fetchone()
#     except Exception,e:
#         pass
...
#     db.commit()
    while line:
        t = line.split("\t")
        if len(t) == 7 and t[0] != '      日期':
            try:
                t[0] = "%s 00:00:00" % (t[0]) #yyyy-MM-dd HH:mm 
#                 print t[0],len(t[0])
...
#                 return
                cursor = db.cursor()            
                sql = "insert DAYZS (DM,JYRQ,KP,ZG,ZD,SP,CJL,CJE) values('%s','%s',%s,%s,%s,%s,%s,%s);" % (dm,t[0],t[1],t[2],t[3],t[4],t[5],t[6])
#                 print sql
                cursor.execute(sql)
                cursor.close()
                db.commit()
...
#     print fd
    files = listfilename("/mnt/windows/gp/D1/ZS")
    for file in files:
        filetodb(file)
