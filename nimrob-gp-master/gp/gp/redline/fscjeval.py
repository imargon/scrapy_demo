...
#             sql = sql.encode("utf8")
        ...
        try:
            ...
            sql = '''
               ...
               d.dlsdebz,d.dlsdlbz,d.dlsdfsbz,floor(d.nzfz) as nzfz,d.zfz/10.0 as zfz,
               d.upepj/d.downepj as epj,d.uplpj/d.downlpj as lpj,d.uptpj/d.downtpj as tpj,
               d.mbs/d.mss as mbs,d.mups/d.mdowns as muds,d.hsl * 10 as hsl,d.nb,d.jlr/d.cje * 10 as jlr
               from fscjmxhz_mem as d 
               where d.jyrq >= '%s'  and d.jyrq <= '%s' and d.jyrq !='%s'; 
                ''' % (jyrq,jyrq2,currdate)
#             print sql
#             sql = sql.encode("utf8")
            cursor.execute(sql)
            ...
            while row is not None:
                ...
                reMat[index,38] = float(row["zfz"])
                reMat[index,39] = float(row["epj"])
                reMat[index,40] = float(row["lpj"])
                reMat[index,41] = float(row["tpj"])
                reMat[index,42] = float(row["mbs"])
                reMat[index,43] = float(row["muds"])
                reMat[index,44] = float(row["hsl"])
                reMat[index,45] = float(row["nb"])
                reMat[index,46] = float(row["jlr"])
...
def selectTopTest(jyrq,rdl):
        ...
        try:
            ...
            sql = '''
               ...
               a.dlsdebz,a.dlsdlbz,a.dlsdfsbz,floor(a.nzfz) as nzfz,a.zfz/10.0 as zfz,
...
def classifyTest(ybsd,ybed,csd,my):
    ...
    P100,MB) VALUES('%s','%s',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);
                    ''' % (tdm[zj],sdate,gl["P00"]/zfzcs,gl["P01"]/zfzcs,gl["P02"]/zfzcs,gl["P03"]/zfzcs,gl["P04"]/zfzcs,gl["P05"]/zfzcs,gl["P06"]/zfzcs,gl["P07"]/zfzcs,gl["P08"]/zfzcs,gl["P09"]/zfzcs,gl["P010"]/zfzcs,gl["P10"]/zfzcs,gl["P20"]/zfzcs,gl["P30"]/zfzcs,gl["P40"]/zfzcs,gl["P50"]/zfzcs,gl["P60"]/zfzcs,gl["P70"]/zfzcs,gl["P80"]/zfzcs,gl["P90"]/zfzcs,gl["P100"]/zfzcs,cstl)
    #                 print sql
    #             sql = sql.encode("utf8")
                cursor.execute(sql)
                cursor.close()
...
def fj(currToday):
    classifyTest('20150106',currToday,currToday,[])
...
#         while row is not None:
#             csmbdate = row["JYRQ"]
#             classifyTest('20150106',curr,csmbdate,[])
#             row = cursor.fetchone()
#         cursor.close()
#         db.commit()
#     except Exception,e:
